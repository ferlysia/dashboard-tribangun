...
function DetailModal({ project, initDetail, onClose, onDetailSaved }: {
  ...
  const [schedEndWeek,   setSchedEndWeek]   = React.useState(1)
  ...
  const [activeLogWeek,  setActiveLogWeek]  = React.useState<number | null>(null)

  // ── Progress 90% alert ───────────────────────────────────────────────────
  const [progressAlert, setProgressAlert] = React.useState<string[] | null>(null)

  // Trigger ketika editProg >= 90: auto-pilih termin pertama yang terbuka → Finance unlock
  React.useEffect(() => {
    if (editProg < 90) { setProgressAlert(null); return }
    const unlocked = terminSchedule.filter(t => editProg >= t.target_progres)
    if (terminSchedule.length > 0 && unlocked.length > 0) {
      setProgressAlert(unlocked.map(t => t.nama))
      setSelectedTerminId(prev => prev ?? unlocked[0].id)
    } else if (terminSchedule.length === 0 && editProg >= 100) {
      setProgressAlert(["Finance terbuka — progress 100% tercapai"])
    } else {
      setProgressAlert(null)
    }
  }, [editProg, terminSchedule])

  // Trigger dari sisi Jadwal: ketika bobot selesai jadwal >= 90%
  React.useEffect(() => {
    const totalW = schedItems.reduce((s, i) => s + i.progress_weight, 0) || 1
    const doneW  = schedItems.filter(i => i.is_done).reduce((s, i) => s + i.progress_weight, 0)
    if ((doneW / totalW) * 100 < 90) return
    const unlocked = terminSchedule.filter(t => editProg >= t.target_progres)
    if (unlocked.length > 0) setSelectedTerminId(prev => prev ?? unlocked[0].id)
  }, [schedItems, terminSchedule, editProg])
  ...
  async function addLog() {
    ...
    try {
      ...
      const r = await fetch(`/api/project-weekly-logs/${encodeURIComponent(project.id)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ week_number: logWeek, description: logDesc.trim(), photo_url: photoUrl, created_by: user?.email || "", progress_pct: editProg }),
      })
      const d = await r.json()
      if (d.data) {
  ...
  async function addSchedItem() {
    ...
    try {
      const r = await fetch(`/api/project-schedule/${encodeURIComponent(project.id)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ week_number: schedWeek, task_description: schedTask.trim(), progress_weight: schedWeight, end_week: schedEndWeek }),
      })
      const d = await r.json()
      if (d.data) {
        setSchedItems(p => [...p, d.data].sort((a, b) => a.week_number - b.week_number))
        setSchedTask(""); setSchedWeek(1); setSchedEndWeek(1); setSchedWeight(10); setShowAddSched(false)
      }
              ...
              <div className="mt-6">
                ...
                {docSubTab === "log" && (
                  <div>
                    ...
                    {/* Log Kanban Board — week tabs + filtered grid */}
                    {docDataLoading ? (
                      <p className="text-xs text-neutral-400 py-6 text-center">Memuat log…</p>
                    ) : weeklyLogs.length === 0 ? (
                      <div className="text-center py-12">
                        <ClipboardList className="h-9 w-9 mx-auto mb-3 text-neutral-200" />
                        <p className="text-sm font-medium text-neutral-500">Belum ada log mingguan</p>
                        <p className="text-xs text-neutral-400 mt-1">Mulai tambah laporan lapangan pertama.</p>
                      </div>
                    ) : (() => {
                      const uniqueLogWeeks = [...new Set(weeklyLogs.map(l => l.week_number))].sort((a, b) => a - b)
                      const visibleLogs   = activeLogWeek !== null
                        ? weeklyLogs.filter(l => l.week_number === activeLogWeek)
                        : weeklyLogs
                      return (
                        <div>
                          {/* Week tab pills */}
                          <div className="flex gap-1.5 flex-wrap mb-4">
                            {uniqueLogWeeks.map(w => (
                              <button key={w} type="button"
                                onClick={() => setActiveLogWeek(activeLogWeek === w ? null : w)}
                                className={`px-3 py-1.5 rounded-full text-[11px] font-bold transition-all border ${
                                  activeLogWeek === w
                                    ? "bg-indigo-600 text-white border-indigo-600 shadow-sm"
                                    : "bg-white text-neutral-500 border-neutral-200 hover:border-indigo-300 hover:text-indigo-600"
                                }`}>
                                Minggu {w}
                              </button>
                            ))}
                            {activeLogWeek !== null && (
                              <button type="button"
                                onClick={() => setActiveLogWeek(null)}
                                className="px-3 py-1.5 rounded-full text-[11px] font-medium text-neutral-400 hover:text-neutral-600 border border-transparent transition-colors">
                                Lihat Semua
                              </button>
                            )}
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {visibleLogs.map(log => {
                              const pct = log.progress_pct ?? editProg
                              const progColor = pct >= 80 ? "#10b981" : pct >= 40 ? "#6366f1" : "#f59e0b"
                              return (
                                <div key={log.id}
                                  className="group relative flex flex-col rounded-xl bg-white border border-neutral-200 hover:border-neutral-300 hover:shadow-sm transition-all duration-200"
                                  style={{ minHeight: 180 }}>

                                  {/* Card top: week label + progress title */}
                                  <div className="px-4 pt-4 pb-3">
                                    <span className="inline-block text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 border border-blue-100 mb-2">
                                      Minggu {log.week_number}
                                    </span>
                                    <p className="text-[13px] font-semibold text-neutral-800 leading-snug">
                                      Minggu {log.week_number}: Progress Fisik {pct}%
                                    </p>
                                    {/* Micro progress bar */}
                                    <div className="mt-2 h-1.5 w-full bg-neutral-100 rounded overflow-hidden">
                                  ...
                                  <div className="px-4 flex-1">
                                    <p className="text-[10px] font-semibold uppercase tracking-wider text-neutral-300 mb-1.5">Catatan</p>
                                    <p className="text-xs text-neutral-600 leading-relaxed"
                                      style={{ display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical" as const, overflow: "hidden" }}>
                                      {log.description || <span className="italic text-neutral-300">Tidak ada catatan.</span>}
                                    </p>
                                  </div>

                                  {/* Card bottom: timestamp + hover utility icons */}
                                  <div className="px-4 py-3 mt-3 border-t border-neutral-100 flex items-center justify-between">
                                    ...
                                    {/* Utility icons — only visible on hover */}
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                      {log.photo_url && (
                                        <a href={log.photo_url} target="_blank" rel="noopener noreferrer"
                                          title="Lihat foto bukti"
                                          className="p-1 rounded hover:bg-neutral-100 text-neutral-300 hover:text-blue-500 transition-colors">
                                          <Eye className="h-3.5 w-3.5" />
                                        </a>
                                      )}
                                      <button type="button" title="Edit log"
                                        className="p-1 rounded hover:bg-neutral-100 text-neutral-300 hover:text-neutral-600 transition-colors">
                                        <Pencil className="h-3.5 w-3.5" />
                                      </button>
                                      <button type="button" title="Hapus log"
                ...
                {docSubTab === "schedule" && (
                  <div>
                    {!showAddSched ? (
                      ...
                      <div className="rounded-xl border border-border p-4 mb-5" style={{ background: "color-mix(in oklch, var(--muted) 15%, transparent)" }}>
                        ...
                        <div className="grid gap-3 sm:grid-cols-3 mb-3">
                          <div>
                            <label className="text-[11px] text-muted-foreground mb-1 block font-semibold">Mulai Minggu ke-</label>
                            <select className="minput" title="Pilih minggu mulai" style={{ fontSize: 12 }}
                              value={schedWeek} onChange={e => { const v = Number(e.target.value); setSchedWeek(v); if (schedEndWeek < v) setSchedEndWeek(v) }}>
                              {Array.from({ length: 20 }, (_, i) => i + 1).map(w => (
                                <option key={w} value={w}>Week {w}</option>
                              ))}
                            </select>
                          ...
                          <div>
                            <label className="text-[11px] text-muted-foreground mb-1 block font-semibold">Sampai Minggu ke-</label>
                            <select className="minput" title="Pilih minggu selesai" style={{ fontSize: 12 }}
                              value={schedEndWeek} onChange={e => setSchedEndWeek(Number(e.target.value))}>
                              {Array.from({ length: 20 }, (_, i) => i + 1).filter(w => w >= schedWeek).map(w => (
                                <option key={w} value={w}>Week {w}</option>
                              ))}
                            </select>
                          </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button type="button"
                            onClick={() => { setShowAddSched(false); setSchedTask(""); setSchedWeek(1); setSchedEndWeek(1); setSchedWeight(10) }}
                            className="savebtn" style={{ fontSize: 12, padding: "7px 16px", background: "var(--muted)", color: "var(--muted-foreground)" }}>
                            Batal
                          </button>
                        </div>
                    ...
                    {/* Jadwal Gantt — Multi-Week Timeline */}
                    {docDataLoading ? (
                      <p className="text-xs text-neutral-400 py-6 text-center">Memuat jadwal…</p>
                    ) : schedItems.length === 0 ? (
                      <div className="text-center py-12">
                        <ListChecks className="h-9 w-9 mx-auto mb-3 text-neutral-200" />
                        <p className="text-sm font-medium text-neutral-500">Belum ada rencana tugas</p>
                        <p className="text-xs text-neutral-400 mt-1">Tambahkan fase kerja mingguan untuk memulai timeline.</p>
                      </div>
                    ) : (() => {
                      const PILL = [
                        { bg: "#d4edda", text: "#1e5c35", done: "#a3cfbb" },
                        { bg: "#d6eaf8", text: "#1a4f72", done: "#85c1e9" },
                        { bg: "#f5e6d3", text: "#7d5a3c", done: "#e8c99a" },
                        { bg: "#ead6f5", text: "#6b3fa0", done: "#c9a0e0" },
                        { bg: "#fef9e7", text: "#7d6608", done: "#f8d775" },
                        { bg: "#fde8e8", text: "#922b21", done: "#f1948a" },
                      ]
                      ...
                      const maxWeek     = Math.max(...schedItems.map(i => Math.max(i.week_number, i.end_week || i.week_number)), 1)
                      const weeks       = Array.from({ length: Math.max(maxWeek, 10) }, (_, k) => k + 1)
                      const COL_W       = 72
                      const baseMonth   = schedItems.length > 0 ? new Date(schedItems[0].created_at).getMonth() : new Date().getMonth()
                      const monthGrps: { label: string; count: number }[] = []
                      let wIdx = 0, mOff = 0
                      while (wIdx < weeks.length) {
                        const cnt = Math.min(4, weeks.length - wIdx)
                        monthGrps.push({ label: MONTHS_ID[(baseMonth + mOff) % 12], count: cnt })
                        wIdx += 4; mOff++
                      }
                      ...
                      return (
                        <div>
                          {/* Summary strip */}
                          <div className="flex items-center justify-between mb-3 px-3.5 py-2.5 rounded-xl bg-neutral-50 border border-neutral-200">
                            <span className="text-xs text-neutral-500">
                              {schedItems.filter(s => s.is_done).length}/{schedItems.length} fase selesai
                              <span className="ml-2 text-[10px] text-neutral-400">· {doneWeight}% dari {totalWeight}% total bobot</span>
                            </span>
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-bold tabular-nums"
                                style={{ color: donePct >= 80 ? "#059669" : donePct >= 40 ? "#4f46e5" : "#d97706" }}>
                                {donePct}%
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="flex rounded-xl overflow-hidden border border-neutral-200">
                            ...
                            {/* ── LEFT: Phase sidebar ── */}
                            <div className="w-1/4 flex-shrink-0 border-r border-neutral-200">
                              {/* Sidebar header — 2 rows to match month+week headers */}
                              ...
                              {schedItems.map((item, i) => {
                                ...
                                const hasBell = terminSchedule.some(t => {
                                  let cum = 0
                                  for (const si of schedItems) {
                                    const prev = cum; cum += si.progress_weight
                                    if (si.id === item.id) return t.target_progres > prev && t.target_progres <= cum
                                  }
                                  return false
                                })
                                return (
                                  <div key={item.id}
                                    className="group/row flex items-center gap-2 px-3 py-3 border-b border-neutral-200 hover:bg-neutral-50 transition-colors"
                                    style={{ minHeight: 52 }}>
                                    <button type="button"
                                      ...
                                      style={{
                                        background: item.is_done ? c.done : "transparent",
                                        border: `1.5px solid ${item.is_done ? c.done : "#d1d5db"}`,
                                      }}>
                                      {item.is_done && <CheckCheck className="h-2.5 w-2.5 text-white" />}
                                    </button>
                                    ...
                                    <div className="flex-shrink-0 flex items-center gap-1">
                                      {hasBell && (
                                        <span title="Ada syarat termin pembayaran di fase ini"><Bell className="h-3 w-3 text-amber-400" /></span>
                                      )}
                            ...
                            {/* ── RIGHT: Multi-week Gantt ── */}
                            <div className="flex-1 overflow-x-auto" style={{ scrollbarWidth: "thin" }}>
                              <div style={{ minWidth: weeks.length * COL_W }}>

                                {/* Month header row */}
                                <div className="flex border-b border-neutral-200 bg-neutral-50">
                                  {monthGrps.map((mg, mi) => (
                                    <div key={mi} className="flex-shrink-0 flex items-center justify-center border-r border-neutral-200 py-1.5"
                                      style={{ width: COL_W * mg.count }}>
                                      <span className="text-[10px] font-bold text-neutral-500 tracking-widest uppercase">{mg.label}</span>
                                    </div>
                                  ))}
                                </div>
                                ...
                                <div className="flex border-b border-neutral-200 bg-neutral-50">
                                  {weeks.map(w => (
                                    <div key={w} className="flex-shrink-0 flex items-center justify-center py-2 border-r border-neutral-100"
                                      style={{ width: COL_W }}>
                                      <span className="text-[10px] font-bold text-neutral-400">W{w}</span>
                                    </div>
                                  ))}
                                </div>
                                ...
                                {schedItems.map((item, i) => {
                                  const c       = PILL[i % PILL.length]
                                  const startW  = Math.max(1, item.week_number)
                                  const endW    = Math.max(startW, item.end_week || item.week_number)
                                  ...
                                  const labelLeft = barLeft + barWidth + 6
                                  return (
                                    <div key={item.id}
                                      className="relative border-b border-neutral-100"
                                      style={{ minHeight: 52, width: weeks.length * COL_W }}>
                                      {/* Column grid lines */}
                                      ...
                                      {/* Bar */}
                                      <div
                                        className="absolute flex items-center gap-1.5 rounded-full px-3 shadow-sm overflow-hidden"
                                        style={{
                                          left: barLeft,
                                          width: barWidth,
                                          height: 32,
                                          ...
                                          background: item.is_done ? c.done : c.bg,
                                          color: c.text,
                                          zIndex: 1,
                                        }}>
                                        {hasBell && <Bell className="h-3 w-3 text-amber-400 flex-shrink-0" />}
                                        {item.is_done && <CheckCheck className="h-2.5 w-2.5 flex-shrink-0" />}
                                        <span className="text-[10px] font-bold tabular-nums whitespace-nowrap">
                                          {item.progress_weight}%
                                        </span>
                                      </div>
                                      {/* Phase label after bar */}
                                      {labelLeft < weeks.length * COL_W - 20 && (
                                        <div
                                          className="absolute text-[10px] font-medium text-neutral-400 whitespace-nowrap overflow-hidden"
                                          style={{
                                            left: labelLeft,
                                            top: "50%",
                                            transform: "translateY(-50%)",
                                            maxWidth: weeks.length * COL_W - labelLeft - 4,
                                          }}>
                                          {item.task_description}
                                        </div>
                                      )}
