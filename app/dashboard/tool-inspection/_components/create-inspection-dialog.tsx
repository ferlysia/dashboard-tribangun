...
import { useCreateInspection, useCatalog } from "../_hooks/use-tool-inspection"
...
export function CreateInspectionDialog({ open, onClose, siteId, siteName }: {
  open:     boolean
  onClose:  () => void
  siteId:   string
  siteName: string
  ...
  const { data: catalog } = useCatalog(siteId)
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent className="sm:max-w-lg p-0 gap-0">
        <DialogHeader className="border-b border-slate-200 dark:border-slate-800 px-6 py-4">
          <DialogTitle>New Weekly Tool Inspection — {siteName}</DialogTitle>
          <DialogDescription>
            {activeCatalogCount > 0
              ? `${activeCatalogCount} tool akan disalin otomatis dari katalog site ini.`
              : "Site ini belum punya katalog tool aktif — atur katalog dulu sebelum membuat inspeksi."}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-200 dark:border-slate-800 bg-muted/30 px-6 py-4">
          ...
          <Button type="button" size="sm" onClick={handleSubmit} disabled={createInspection.isPending || activeCatalogCount === 0}>
