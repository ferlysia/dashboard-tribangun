...
async function importBatch(rows, batchNumber, totalBatches) {
  const response = await fetch(`${SUPABASE_URL}/rest/v1/invoices?on_conflict=workbook_year,sheet_name,invoice_no`, {
    method: "POST",
    headers: {
...
async function main() {
  ...
  for (let index = 0; index < batches.length; index += 1) {
    await importBatch(batches[index], index + 1, batches.length)
    console.log(`Imported batch ${index + 1}/${batches.length}`)
  }

  const count = await verifyCount()
  console.log(`Supabase invoices count: ${count}`)
}
